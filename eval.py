...
def get_autoais_device():
    requested = os.environ.get("AUTOAIS_DEVICE")
    if requested:
...
def load_autoais_model():
    device = get_autoais_device()
    if device.type == "cuda":
        torch.cuda.empty_cache()
    logger.info("Loading AutoAIS model on %s without disk offload.", device)
...
def encode_autoais_input(text: str):
    device = get_autoais_device()
    encoded = autoais_tokenizer(text, return_tensors="pt")
...
def _run_nli_autoais(passage, claim):
    ...
    inputs = encode_autoais_input(input_text)
    with torch.inference_mode():
        outputs = autoais_model.generate(**inputs, max_new_tokens=10)
...
def compute_claims(data):
    ...
    if autoais_model is None:
        ...
        autoais_model = load_autoais_model()
    ...
    if args.mauve:
        try:
            if autoais_model is not None:
                del globals()["autoais_model"]
                globals()["autoais_model"] = None
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            result['mauve'] = compute_mauve(normalized_data)
        except Exception as exc:
            logger.warning("MAUVE failed; continuing with available task/citation metrics. Error: %s", exc)
