    ...
    def __init__(self, args):
        ...
        if args.openai_api:
            ...
            if args.azure:
                ...
                # For OpenAI-compatible third-party APIs, e.g. https://xxx/v1
                if OPENAI_API_BASE:
                    openai.api_base = OPENAI_API_BASE
            ...
            if use_chat_api:
                ...
                prompt = [
                    {'role': 'system', 'content': (
                        ...
                        "Write one single paragraph. Do not use bullet points or line breaks. "
                        "Every factual sentence must end with one to three citations like [1] or [1][2]. "
                        "Do not cite documents outside the provided range."
                    )},
                    {'role': 'user', 'content': prompt}
                ]
            ...
            if use_chat_api:
                ...
                while not is_ok:
                    ...
                    try:
                        ...
                        if args.azure:
                            chat_kwargs["engine"] = deploy_name
...
def main():
    ...
    parser.add_argument("--no_doc_in_demo", type=str2bool, nargs="?", const=True, default=False, help="Whether to remove the documents in the demos")
    parser.add_argument("--fewer_doc_in_demo", type=str2bool, nargs="?", const=True, default=False, help="Whether to use fewer documents in the demos")
    ...
    parser.add_argument("--openai_api", type=str2bool, nargs="?", const=True, default=False, help="Whether to use OpenAI API")
    ...
    parser.add_argument("--interactive", type=str2bool, nargs="?", const=True, default=False, help="Whether to run in interactive mode")
    ...
    parser.add_argument("--retrieve_in_all_docs", type=str2bool, nargs="?", const=True, default=False, help="Retrieve in all documents instead of just top ndoc")
    ...
    parser.add_argument("--force_cite_show", type=str2bool, nargs="?", const=True, default=False, help="Force citing the documents that are shown to the model")
    ...
    if any(x in model_lower for x in ["turbo", "gpt-4o", "gpt-4", "gpt-3.5"]):
        args.max_length = 4096
    ...
    if "16k" in model_lower:
        ...
        args.max_length = 8192
    elif "turbo" in model_lower or "gpt-3.5" in model_lower:
        args.max_length = 4096
    ...
    for idx, item in enumerate(tqdm(eval_data)):
        ...
        for _ in range(args.num_samples):
            ...
            output_array[-1] = clean_output(output_array[-1])
    ...
    if args.openai_api:
        ...
        if "gpt-4o-mini" in model_lower:
            # Third-party API price may differ; set to 0 to avoid misleading cost estimate.
            p_price, c_price = 0, 0
        elif "turbo" in model_lower:
            p_price, c_price = 0.0015, 0.002
            if "16k" in model_lower:
                p_price, c_price = 0.003, 0.004
        elif "gpt4" in model_lower or "gpt-4" in model_lower or "gpt-4o" in model_lower:
            p_price, c_price = 0, 0
        else:
            logger.warning("Cannot find model price")
            p_price, c_price = 0, 0
