#!/usr/bin/env python3
"""Run claim audit, evaluation, score merging, and summary for result files.

The batch executes these commands for each input:

  1. scripts/claim_audit.py
  2. eval.py
  3. scripts/merge_scores.py
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--device", default=None, help="Optional NLI device passed to claim_audit.py.")
    parser.add_argument("--nli-batch-size", type=int, default=None)
    parser.add_argument("--no-mauve", action="store_true", help="Do not run MAUVE during ALCE eval.py.")
...
def build_eval_command(args: argparse.Namespace, root: Path, cover_result: Path) -> list[str]:
    ...
    if dataset == "asqa":
        cmd.extend(["--citations", "--qa"])
        if not args.no_mauve:
            cmd.append("--mauve")
    elif dataset == "eli5":
        cmd.extend(["--citations", "--claims_nli"])
        if not args.no_mauve:
            cmd.append("--mauve")
