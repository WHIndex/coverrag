#!/usr/bin/env python3
"""Revise attributed RAG answers using evidence-supported atomic claims."""
...
logger = logging.getLogger("answer_revision")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Revise an attributed RAG result with verified claims.")
    ...
    # Claim extraction and verification settings.
    ...
    parser.add_argument(
        ...
        help=(
            "`cited_then_all` first verifies cited evidence, then checks whether "
            "another already-retrieved document supports the claim."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "`auto` preserves claim-citation alignment while adapting to the answer format."
        ),
    )
    ...
    parser.add_argument(
        "--final-audit",
        action="store_true",
        help="Audit the revised output before writing the final result.",
    )
    parser.add_argument(
...
class LLMRevisionGenerator(RevisionGenerator):
    ...
    def revise(
        ...
        key = "answer-revision:" + stable_hash(question, draft_answer, verified_block, rejected_block)
...
def extract_qampari_answer_item(claim: dict[str, Any]) -> str:
    ...
    #
    # Expansion claims use an evidence sentence rather than the answer item.
    # Prefer the explicit answer span so QAMPARI
    # output stays a comma-separated entity list instead of leaking full
    # evidence sentences into the answer.
    candidates = []
    for key in ("answer_span",):
...
def fill_dataset_name_from_payload(args: argparse.Namespace, payload: dict[str, Any]) -> None:
    ...
    treated as a paragraph answer and list-style formatting is lost.
    """
...
def revise_item(
    ...
    if args.final_audit:
        updated["cover_revision"]["final_audit"] = audit_item(updated, item_id, decomposer, verifier, args)
    return updated
...
def summarize_revision(items: Sequence[dict[str, Any]]) -> dict[str, Any]:
    records = [item.get("cover_revision", {}) for item in items if item.get("cover_revision")]
...
def summarize_final_audit(items: Sequence[dict[str, Any]]) -> dict[str, Any] | None:
    final_audits = [item.get("cover_revision", {}).get("final_audit") for item in items]
...
def main() -> None:
    ...
    revised_items = []
    for item_id, item in enumerate(tqdm(process_items, desc="CoverRAG answer revision")):
    ...
    payload["cover_revision_summary"] = summarize_revision(revised_items[: len(process_items)])
    final_summary = summarize_final_audit(revised_items[: len(process_items)])
    if final_summary is not None:
        payload["cover_revision_final_audit_summary"] = final_summary
    payload["cover_revision_config"] = {
    ...
    if token_usage:
        payload["cover_revision_token_usage"] = token_usage
    ...
    logger.info("Wrote CoverRAG revision result to %s", args.output)
    logger.info("CoverRAG revision summary: %s", json.dumps(payload["cover_revision_summary"], ensure_ascii=False, indent=2))
