#!/usr/bin/env bash
# BGE-Rerank RAG base + COVER-RAG v13 plug-in.
#
# This uses the already generated ALCE outputs whose retrieval file was
# reranked by scripts/rerank_alce_docs.py. That reranker is a cross-encoder
# relevance model, defaulting to BAAI/bge-reranker-large, which scores each
# (question, document) pair and reorders the top100 candidates before the
# normal gpt-4o-mini RAG generation.
#
# Launch:
#   nohup bash scripts/run_bge_rerank_rag_cover_nohup.sh \
#     > logs/bge_rerank_rag_cover_v13_$(date +%Y%m%d_%H%M%S).log 2>&1 &
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-bge_rerank_rag_cover_v13}"
export BASE_DIR="${BASE_DIR:-result/${EXPERIMENT_NAME}_full}"
mkdir -p "$BASE_DIR" logs cache

# Fixed reranker-RAG base outputs. Do not regenerate these in this script: v0
# must measure the exact same base output that v3 will repair.
export ASQA_RAW="${ASQA_RAW:-result/origin/asqa-gpt-4o-mini-bge_rerank_default-shot2-ndoc5-42.json}"
export ELI5_RAW="${ELI5_RAW:-result/origin/eli5-gpt-4o-mini-bge_rerank_default-shot2-ndoc5-42.json}"
export QAMPARI_RAW="${QAMPARI_RAW:-result/origin/qampari-gpt-4o-mini-bge_rerank_default-shot2-ndoc5-42.json}"

# Candidate evidence pools are the same BGE-reranked top100 files used by the
# base generation, so COVER can recover/expand from the base method's evidence.
...
# This is a distinct base method from vanilla RAG. Always rebuild v0 unless
# explicitly overridden.
export PREPARE_V0="${RERANK_RAG_PREPARE_V0:-1}"
export V0_SUMMARY_CSV="${RERANK_RAG_V0_SUMMARY_CSV:-}"
export V0_DIR="${V0_DIR:-$BASE_DIR/cover_audit}"
export V0_CACHE_FILE="${V0_CACHE_FILE:-cache/cover_audit_bge_rerank_rag_v13_cache.json}"
export V0_SCORE_MODE="${V0_SCORE_MODE:-eval}"
export FORCE_V0="${FORCE_V0:-0}"

# Frozen v13 COVER-RAG plug-in settings, aligned with the main vanilla and
# self-RAG runs.
export CACHE_NAMESPACE="${CACHE_NAMESPACE:-bge_rerank_rag_v13_}"
...
echo "=== BGE-Rerank RAG + COVER-RAG v13 ==="
echo "Base dir: $BASE_DIR"
echo "OpenAI base: $OPENAI_API_BASE"
echo "Reranker used for base retrieval files: BAAI/bge-reranker-large via scripts/rerank_alce_docs.py"
echo "Inputs:"
