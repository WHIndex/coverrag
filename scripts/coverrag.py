#!/usr/bin/env python3
"""Claim-citation coverage control for attributed RAG.

The controller audits a host answer, routes unsupported claims to evidence
recovery, expands missing answer units from verified evidence, regenerates a
cited answer, and audits the result again. In evidence-seeded mode it builds a
verified claim-citation blueprint before the final reader call.
"""
...
from answer_reader import BlueprintAnswerReader


logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
logger = logging.getLogger("coverrag")
...
class CandidateDocProvider:
    """Load optional candidate documents for retrieval and evidence expansion."""
...
def general_answer_span_type_mismatch(span: str, question: str = "", evidence_sentence: str = "") -> str:
    ...
    if qtype == "explanation":
        # Explanatory answers need mechanism or definition clauses; isolated
        # dates, names, and metadata are usually not useful answer units.
...
def claim_priority_key(claim: dict[str, Any]) -> tuple[int, int, int, int, int]:
    ...
    return (
        ...
        status_rank.get(str(claim.get("revision_status", "")), 2),
...
def verify_doc(
    ...
    result.evidence_scope = "targeted_single_doc"
    return result
...
def recover_claim(
    ...
    if not supported_doc_ids and args.recovery_group_size > 1 and top_ranked:
        ...
        group_result.evidence_scope = "targeted_doc_group"
        attempts.append(
            {
    ...
    recovered["revision_status"] = "recovered"
    recovered["recovered_from_label"] = claim.get("label")
    recovered["recovery"] = diagnostics
    ...
    recovered["evidence_scope"] = "targeted_recovery"
    if best_result is not None:
        ...
        recovered["verifier"] = f"{best_result.verifier}+recovery"
...
def prepare_claims(
    ...
    for claim in raw_claims:
        ...
        if claim_is_initially_usable(claim, args):
            updated = dict(claim)
            updated["revision_status"] = "initial_supported"
        ...
        rejected_claim["revision_status"] = "rejected_after_recovery" if claim_should_recover(claim, args) else "not_recovery_eligible"
    ...
    for claim in overflow:
        updated = dict(claim)
        updated["revision_status"] = "dropped_by_answer_budget"
...
def compute_revision_metrics(
    ...
    initially_supported = sum(1 for claim in verified_claims if claim.get("revision_status") == "initial_supported")
    return {
...
def render_answer_preserving_answer(
    ...
    tail_claims.sort(key=claim_priority_key)
...
def render_span_preserving_answer(
    ...
    for sentence_record in draft_audit.get("sentences", []) or []:
        ...
        partial_claims.sort(key=claim_priority_key)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CoverRAG over an attributed RAG result JSON.")
    ...
    parser.add_argument(
        "--control-mode",
        choices=["answer_seeded", "evidence_seeded"],
        default="answer_seeded",
        help=(
            "answer_seeded audits and revises the host answer; evidence_seeded "
            "plans and verifies claim-citation units before a single final reader call."
        ),
    )
    ...
    parser.add_argument(
        "--reader-conformance-verify",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Verify each realized answer unit against its blueprint-authorized evidence without another reader call.",
    )
...
def claim_is_expansion(claim: dict[str, Any]) -> bool:
    return (
        str(claim.get("revision_status", "") or "") == "expanded"
        or str(claim.get("coverage_status", "") or "") == "expanded_supported"
...
def select_explanation_integrated_claims(
    ...
    candidates = sorted(
        expanded_claims,
        key=lambda claim: (
            -claim_float_feature(claim, "answer_unit_directness"),
            -claim_float_feature(claim, "answer_unit_score"),
            -claim_float_feature(claim, "v3_question_overlap"),
            claim_priority_key(claim),
        ),
    )
    for claim in candidates:
        ...
        if key and base_key_text and key in base_key_text:
            continue
        if claim_float_feature(claim, "answer_unit_score") < threshold:
            continue
        if getattr(args, "answer_unit_relevance_gate", False):
            ...
            if claim_float_feature(claim, "answer_unit_directness") < min_directness:
                continue
...
def expansion_answer_unit_score_sum(claims: Sequence[dict[str, Any]]) -> float:
    return sum(claim_float_feature(claim, "answer_unit_score") for claim in claims)
...
def qampari_claim_is_answer_relevant(claim: dict[str, Any], question: str) -> tuple[bool, str]:
    ...
    bad, reason = qampari_is_bad_answer_span(span, question, evidence_sentence=evidence if claim.get("revision_status") == "expanded" else "")
    if bad:
        return False, reason
    return True, ""
...
def filter_qampari_claims_for_answer_relevance(
    ...
    for claim in claims:
        ...
        if not ok:
            dropped = dict(claim)
            dropped["coverage_status"] = "dropped_by_answer_relevance"
            dropped["answer_relevance_reason"] = reason
            rejected.append(dropped)
            continue
        if key in seen:
            dropped = dict(claim)
            dropped["coverage_status"] = "dropped_duplicate_answer_item"
        ...
        for sent_id, sentence in enumerate(sentences):
            ...
            if doc.get("cover_dynamic_retrieval_score") is not None:
                ...
                try:
                    ...
                    pass
            if getattr(args, "answer_targeted_expansion", False) and infer_answer_format(args) != "qampari":
                # Reward compact evidence clauses that add a question-relevant
                # answer unit instead of supported background alone.
    ...
    for target in targets:
        ...
        if target_key and target_key in candidate_key:
            ...
            if str(target.get("source", "")) == "ranked_evidence":
                ...
                # answer units, so they cannot prove their own target match.
...
class LLMExpansionGenerator(ExpansionCandidateGenerator):
    ...
    def generate(
        ...
        key = "coverrag-expand:" + stable_hash(
            question,
            supported_answer,
            supported,
            rejected,
...
def verify_expansion_candidate(
    ...
    result.evidence_scope = "expansion_single_doc"
    diagnostics = {
        "attempted": True,
        "doc_id": doc_id,
        "label": result.label,
    ...
    claim_record = {
        "claim_id": f"expanded-{stable_hash(question, claim, str(doc_id))[:12]}",
        "claim": claim,
        "importance": "critical",
        "claim_type": "answer_expansion",
        "claim_extractor": f"expansion_{candidate.get('source', args.expansion_mode)}",
        ...
        "answer_unit_score": float(candidate.get("answer_unit_score", 0.0) or 0.0),
        "answer_unit_score_parts": candidate.get("answer_unit_score_parts", {}),
        "answer_unit_directness": float(relevance_diagnostics.get("answer_unit_directness", 0.0) or 0.0),
        "answer_unit_question_relevance": float(
            relevance_diagnostics.get("answer_unit_question_relevance", 0.0) or 0.0
        ),
        "answer_unit_target_relevance": float(
            relevance_diagnostics.get("answer_unit_target_relevance", 0.0) or 0.0
        ),
        "answer_unit_has_explanatory_signal": bool(
...
def expand_verified_claims(
    ...
    if args.preserve_first_pass_claims:
        ...
        combined.sort(key=claim_priority_key)
...
def tag_expansion_round(
    ...
    *,
    ...
    for claim in claims:
        row = dict(claim)
        row["iteration_round"] = round_index
        ...
        round_overflow = [
            {
                **dict(claim),
                ...
                "expansion_phase": "iterative_completion_overflow",
            }
...
def compute_metrics(
    ...
    iterative_expanded = [claim for claim in expanded_claims if int(claim.get("iteration_round", 0) or 0) > 0]
...
def summarize_coverrag(items: Sequence[dict[str, Any]]) -> dict[str, Any]:
    records = [item.get("coverrag", {}) for item in items if item.get("coverrag")]
    ...
    control_modes: collections.Counter[str] = collections.Counter(
        str(record.get("control_mode", "answer_seeded") or "answer_seeded") for record in records
    )
    total_evidence_seed_claims = sum(int(m.get("evidence_seed_claims", 0)) for m in metrics)
    total_blueprint_claims = sum(int(m.get("blueprint_claims", 0)) for m in metrics)
    total_reader_api_calls = sum(int(m.get("reader_api_calls", 0)) for m in metrics)
    total_reader_fallback_units = sum(int(m.get("reader_fallback_units", 0)) for m in metrics)
    total_reader_accepted_units = sum(int(m.get("reader_accepted_units", 0)) for m in metrics)
    ...
    return {
        ...
        "control_mode_counts": dict(control_modes),
        "evidence_seed_claims": total_evidence_seed_claims,
        "blueprint_claims": total_blueprint_claims,
        "avg_blueprint_claims": total_blueprint_claims / len(records) if records else 0.0,
        "reader_api_calls": total_reader_api_calls,
        "reader_api_calls_per_example": total_reader_api_calls / len(records) if records else 0.0,
        "reader_accepted_units": total_reader_accepted_units,
        "reader_fallback_units": total_reader_fallback_units,
...
def summarize_final_audit(items: Sequence[dict[str, Any]]) -> dict[str, Any] | None:
    final_audits = [item.get("coverrag", {}).get("final_audit") for item in items]
...
class RevisionWorkerContext:
    def __init__(self, args: argparse.Namespace, cache: JsonCache) -> None:
        ...
        self._states: list[tuple[Any, Any, ExpansionCandidateGenerator, BlueprintAnswerReader | None]] = []
    ...
    def get(self) -> tuple[Any, Any, ExpansionCandidateGenerator, BlueprintAnswerReader | None]:
        ...
        if state is None:
            ...
            blueprint_reader = None
            if self.args.control_mode == "evidence_seeded":
                blueprint_reader = BlueprintAnswerReader(
                    self.args.reader_model,
                    self.cache,
                    temperature=self.args.llm_temperature,
                    top_p=self.args.llm_top_p,
                    max_retries=self.args.llm_max_retries,
                )
            state = (decomposer, verifier, expansion_generator, blueprint_reader)
            self._local.state = state
            with self._states_lock:
                self._states.append(state)
        return state

    def states(self) -> list[tuple[Any, Any, ExpansionCandidateGenerator, BlueprintAnswerReader | None]]:
        with self._states_lock:
...
def collect_worker_token_usage(worker_context: RevisionWorkerContext) -> dict[str, int]:
    ...
    seen_blueprint_readers: set[int] = set()
    placeholder = object()
    revision_generator = AutoRevisionGenerator()

    for decomposer, verifier, expansion_generator, blueprint_reader in worker_context.states():
        ...
        if blueprint_reader is not None and id(blueprint_reader) not in seen_blueprint_readers:
            token_usage["reader_prompt_tokens"] = token_usage.get("reader_prompt_tokens", 0) + int(
                blueprint_reader.prompt_tokens
            )
            token_usage["reader_completion_tokens"] = token_usage.get("reader_completion_tokens", 0) + int(
                blueprint_reader.completion_tokens
            )
            token_usage["reader_api_calls"] = token_usage.get("reader_api_calls", 0) + int(blueprint_reader.api_calls)
            seen_blueprint_readers.add(id(blueprint_reader))
    return token_usage


def empty_evidence_seed_audit(item: dict[str, Any], item_id: int, args: argparse.Namespace) -> dict[str, Any]:
    return {
        "item_id": item_id,
        "question": str(item.get(args.question_field, "") or ""),
        "answer": "",
...
def evidence_seed_args(args: argparse.Namespace) -> argparse.Namespace:
    seed_args = copy.copy(args)
    seed_claims = max(1, min(int(args.evidence_seed_max_claims), final_claim_limit(args)))
    seed_args.max_expanded_claims = seed_claims
    seed_args.max_explanatory_expanded_claims = seed_claims
    seed_args.max_expansion_verifications = max(1, int(args.evidence_seed_max_verifications))
    seed_args.max_explanatory_expansion_verifications = max(1, int(args.evidence_seed_max_verifications))
    seed_args.expansion_evidence_sentences = max(1, int(args.evidence_seed_evidence_sentences))
    seed_args.expansion_candidate_spans = max(1, int(args.evidence_seed_candidate_spans))
    seed_args.max_expansion_candidates = max(1, int(args.evidence_seed_max_candidates))
    seed_args.max_selected_span_ids = max(seed_claims, int(getattr(args, "max_selected_span_ids", 8) or 8))
    seed_args.recall_oriented_completion = True
    seed_args.expansion_use_intermediate_answer = False
    seed_args.preserve_first_pass_claims = True
    return seed_args


def revise_item_evidence_first(
    item: dict[str, Any],
    item_id: int,
    ...
    """Plan verified claim-citation units first, then call the reader once."""
    ...
    # The initial plan sees the host RAG evidence only.  Candidate-pool evidence
    # enters later through gap-conditioned retrieval, preserving the controller
    # distinction between initial evidence and corrective evidence acquisition.
    updated[args.docs_field] = merged_doc_pool(raw_docs, [], args)
    draft_audit = empty_evidence_seed_audit(updated, item_id, args)
    seed_args = evidence_seed_args(args)

    verified_claims, seed_claims, expansion_attempts, expansion_overflow = expand_verified_claims(
        updated,
        draft_audit,
        [],
        [],
    ...
    seed_claims, expansion_attempts = tag_expansion_round(
        seed_claims,
        expansion_attempts,
        round_index=0,
        phase="evidence_seed",
    )
    seed_by_key = {normalize_revision_claim_key(claim, args): claim for claim in seed_claims}
    verified_claims = [seed_by_key.get(normalize_revision_claim_key(claim, args), claim) for claim in verified_claims]
    expanded_claims = list(seed_claims)
    ...
    if int(getattr(args, "iterative_completion_rounds", 0) or 0) > 0:
        ...
        expanded_claims.extend(iterative_claims)
        expansion_attempts.extend(iterative_attempts)
        expansion_overflow.extend(iterative_overflow)
    ...
    verified_keys = {normalize_revision_claim_key(claim, args) for claim in verified_claims}
    expanded_claims = [
        claim for claim in expanded_claims if normalize_revision_claim_key(claim, args) in verified_keys
    ]

    reader_args = copy.copy(args)
    reader_args.answer_format = infer_answer_format(args)
    answer, blueprint, reader_diagnostics = blueprint_reader.realize(
        str(updated.get(args.question_field, "") or ""),
        verified_claims,
        list(updated.get(args.docs_field, []) or []),
        verifier,
        reader_args,
    )
    updated[args.output_field] = answer

    overflow_rejected = [{**dict(claim), "coverage_status": "dropped_by_answer_budget"} for claim in expansion_overflow]
    final_doc_pool = list(updated.get(args.docs_field, []) or [])
    revision_metrics = compute_metrics(
        draft_audit,
        verified_claims,
    ...
    revision_metrics.update(
        {
            "evidence_seed_claims": len(seed_claims),
            "blueprint_claims": len(blueprint),
            "reader_api_calls": int(reader_diagnostics.get("api_calls", 0) or 0),
            "reader_fallback_units": int(reader_diagnostics.get("fallback_units", 0) or 0),
            "reader_accepted_units": int(reader_diagnostics.get("accepted_generated_units", 0) or 0),
        }
    )
    updated["coverrag"] = {
        "control_mode": "evidence_seeded",
        ...
        "first_pass_verified_claims": seed_claims,
        ...
        "render_diagnostics": {"mode": "blueprint_single_call"},
        "candidate_selection_diagnostics": {},
        "revision_metrics": revision_metrics,
    }
    if args.final_audit:
        # This audit is measurement only. Its result cannot trigger retrieval or
        # another reader call, so the evidence-first transition remains single-shot.
        updated["coverrag"]["final_audit"] = audit_item(updated, item_id, decomposer, verifier, args)
    return updated
...
def revise_item(
    ...
    verifier: Any,
    expansion_generator: ExpansionCandidateGenerator,
    blueprint_reader: BlueprintAnswerReader | None,
    args: argparse.Namespace,
) -> dict[str, Any]:
    if args.control_mode == "evidence_seeded":
        if blueprint_reader is None:
            raise RuntimeError("evidence_seeded control requires a blueprint reader")
        return revise_item_evidence_first(
            item,
            item_id,
            candidate_provider,
            decomposer,
    ...
    updated["coverrag_original_output"] = draft_output
    raw_docs = list(updated.get(args.docs_field, []) or [])
    candidate_docs = candidate_provider.docs_for(updated, item_id)
    ...
    first_pass_claims, rejected_claims, recovered_claims, recovery_attempts = prepare_claims(
        draft_audit,
        updated,
        verifier,
        args,
    ...
    if (
        ...
        logger.warning("Falling back to deterministic atomic revision for item %s because the output was citation-invalid.", item_id)
        revised_output = AutoRevisionGenerator().revise(updated, verified_claims, rejected_claims, args)
    ...
    if candidate_output_selection_applies(updated, args):
        ...
        revised_output = clean_final_answer(revised_output, num_docs=len(updated.get(args.docs_field, []) or []))
    updated.pop("coverrag_original_output", None)

    overflow_rejected = []
    for claim in expansion_overflow:
        claim = dict(claim)
        claim["coverage_status"] = "dropped_by_answer_budget"
    ...
    if args.final_audit:
        updated["coverrag"]["final_audit"] = selected_final_audit or audit_item(updated, item_id, decomposer, verifier, args)
    return updated
...
def write_checkpoint_payload(
    ...
    checkpoint_payload["coverrag_checkpoint"] = {
        "processed_examples": processed_count,
        "total_examples": len(checkpoint_data),
        "final": False,
    }
    checkpoint_payload["coverrag_summary"] = summarize_coverrag(revised_items)
    final_summary = summarize_final_audit(revised_items)
    if final_summary is not None:
        checkpoint_payload["coverrag_final_audit_summary"] = final_summary
    ...
    logger.info("Wrote CoverRAG checkpoint after %d examples to %s", processed_count, checkpoint_path)
...
def revise_items(
    ...
    def revise_one(item_id: int, item: dict[str, Any]) -> tuple[int, dict[str, Any]]:
        decomposer, verifier, expansion_generator, blueprint_reader = worker_context.get()
        return item_id, revise_item(
            item,
            item_id,
            candidate_provider,
            decomposer,
    ...
    if workers <= 1 or len(process_items) <= 1:
        revised_items = []
        for item_id, item in enumerate(tqdm(process_items, desc="CoverRAG")):
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="CoverRAG"):
...
def main() -> None:
    ...
    if args.control_mode == "evidence_seeded" and not args.openai_api:
        raise ValueError("`--control-mode evidence_seeded` requires `--openai-api` for the final reader call.")
    ...
    revised_items = revise_items(process_items, items, payload, candidate_provider, worker_context, cache, args, tqdm)
    cache.flush()
    ...
    payload["coverrag_summary"] = summarize_coverrag(revised_items[: len(process_items)])
    final_summary = summarize_final_audit(revised_items[: len(process_items)])
    if final_summary is not None:
        payload["coverrag_final_audit_summary"] = final_summary
    payload["coverrag_config"] = {
        "control_mode": args.control_mode,
        "reader_model": args.reader_model if args.control_mode == "evidence_seeded" else None,
        "reader_max_tokens": args.reader_max_tokens,
        "reader_max_plan_claims": args.reader_max_plan_claims,
        "reader_max_unit_words": args.reader_max_unit_words,
        "reader_conformance_verify": args.reader_conformance_verify,
        "evidence_seed_max_claims": args.evidence_seed_max_claims,
        "evidence_seed_max_verifications": args.evidence_seed_max_verifications,
        "evidence_seed_evidence_sentences": args.evidence_seed_evidence_sentences,
        "evidence_seed_candidate_spans": args.evidence_seed_candidate_spans,
        "evidence_seed_max_candidates": args.evidence_seed_max_candidates,
    ...
    if token_usage:
        payload["coverrag_token_usage"] = token_usage
    ...
    logger.info("Wrote CoverRAG result to %s", args.output)
    logger.info("CoverRAG summary: %s", json.dumps(payload["coverrag_summary"], ensure_ascii=False, indent=2))
