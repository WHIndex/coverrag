#!/usr/bin/env python3
"""Merge ALCE `.score` metrics with CoverRAG audit and revision metrics.

ALCE's `eval.py` remains the source of metrics such as QA-F1, citation recall,
and citation precision. CoverRAG adds fields such as:
the result JSON, for example:

    cover_audit_summary
    cover_revision_summary
    cover_revision_final_audit_summary
...
def main() -> None:
    ...
    for key in [
        ...
        "cover_revision_summary",
        "cover_revision_final_audit_summary",
        "cover_revision_config",
        "cover_revision_token_usage",
        "coverrag_summary",
        "coverrag_final_audit_summary",
        "coverrag_config",
        "coverrag_token_usage",
