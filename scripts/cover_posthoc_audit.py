#!/usr/bin/env python3
"""COVER-RAG v0: post-hoc atomic-claim citation audit for ALCE outputs.

This script reads an ALCE-style result JSON produced by `run.py` or
`cover_run.py`, decomposes each generated answer into atomic factual claims,
and verifies whether each claim is supported by the documents cited in the
source sentence.

The output remains ALCE-compatible: it preserves the original `args` and
`data` fields, and adds:

    item["cover_audit"]
    payload["cover_audit_summary"]

Recommended first run:

    python scripts/cover_posthoc_audit.py \
      --input result/asqa-...json \
      --output result/asqa-...cover_audit.json \
      --openai-api \
      --decomposer llm \
...
logger = logging.getLogger("cover_posthoc_audit")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--decomposer",
        choices=["llm", "sentence", "claimify"],
        default="llm",
        help=(
            "Claim decomposition method. `claimify` is the recommended high-precision "
            "selection/disambiguation/decomposition pipeline."
        ),
    )
    ...
    parser.add_argument("--lexical-threshold", type=float, default=0.45, help="Only for smoke-test lexical verifier.")
...
class ClaimifyClaimDecomposer(ClaimDecomposer):
    """Claimify-style three-stage decomposer.

    This wraps `scripts/claimify_extractor`, which performs:
    selection -> disambiguation -> decomposition.
    """

    def __init__(self, client: OpenAIChatClient, cache: JsonCache, args: argparse.Namespace) -> None:
        if args.claimify_implementation == "external":
            if args.claimify_external_path is None:
                raise ValueError("`--claimify-implementation external` requires `--claimify-external-path`.")
            from claimify_extractor import ExternalClaimifyExtractor

            self.extractor = ExternalClaimifyExtractor(
                client=client,
                repo_path=args.claimify_external_path,
                cache=cache,
                context_before=args.claimify_context_before,
                context_after=args.claimify_context_after,
                max_stage_retries=args.claimify_max_stage_retries,
                selection_completions=args.claimify_selection_completions,
                selection_min_successes=args.claimify_selection_min_successes,
                disambiguation_completions=args.claimify_disambiguation_completions,
                disambiguation_min_successes=args.claimify_disambiguation_min_successes,
                decomposition_completions=args.claimify_decomposition_completions,
                decomposition_min_successes=args.claimify_decomposition_min_successes,
                use_external_defaults=args.claimify_use_external_defaults,
            )
        else:
            from claimify_extractor import ClaimifyExtractor

            self.extractor = ClaimifyExtractor(
                client=client,
                cache=cache,
                context_before=args.claimify_context_before,
                context_after=args.claimify_context_after,
    ...
    def prompt_tokens(self) -> int:
        return self.extractor.prompt_tokens

    @property
    def completion_tokens(self) -> int:
        return self.extractor.completion_tokens

    def decompose(self, question: str, answer: str, sentence: str) -> list[dict[str, Any]]:
        return self.extractor.extract_sentence(question=question, answer=answer, sentence=sentence)
...
class LexicalVerifier(Verifier):
    ...
    def verify(
        ...
        return VerificationResult(
            label=label,
            confidence=score,
            rationale="Lexical-overlap smoke-test verifier; use NLI or LLM for final experiments.",
            verifier="lexical",
...
def make_decomposer(args: argparse.Namespace, cache: JsonCache) -> ClaimDecomposer:
    ...
    if args.decomposer == "claimify":
        if not args.openai_api:
            raise ValueError("`--decomposer claimify` requires `--openai-api`.")
        ...
        return ClaimifyClaimDecomposer(client, cache, args)
    if not args.openai_api:
    ...
    for sent_id, sentence in enumerate(sentences):
        ...
        for claim_offset, claim_payload in enumerate(claims):
            ...
            claim_record = {
                ...
                "claimify_vote_count": claim_payload.get("claimify_vote_count"),
                "claimify_num_completions": claim_payload.get("claimify_num_completions"),
...
def main() -> None:
    ...
    if args.decomposer in {"llm", "claimify"} and not args.openai_api:
        raise ValueError(
            ...
            "Use `--decomposer sentence` for no-API smoke tests."
        )
    ...
    audited_items = []
    for item_id, item in enumerate(tqdm(process_items, desc="COVER-RAG v0 audit")):
        updated = dict(item)
    ...
    logger.info("Wrote COVER-RAG v0 audit to %s", args.output)
